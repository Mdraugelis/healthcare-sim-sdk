...
The Healthcare Intervention Simulation SDK is a Monte Carlo engine. It exists for problems that need entity-level fidelity — per-seed variance, operator dynamics, score-distribution-dependent decisions, and counterfactual design checks that closed-form methods cannot represent well.

This document describes the **Analytical-First Principle**: before building a scenario, evaluate whether the primary decision question can be answered with closed-form math. If it can, do the analytical pass first, document assumptions and limits, and escalate to simulation only if gaps remain decision-critical.
...
Before any code, write one sentence that starts with "We will deploy / not deploy / escalate / stop this AI if …". If the sentence cannot be written, stop and go back to the stakeholder. You are not ready to model.
...
For a factorial design with factors A, B, C, D, E, the sweep is a handful of nested loops around `itertools.product` and the two-proportion z-test. A complete working example is maintained in a private external repository and is intentionally not linked here.
...
A concrete reference implementation of the Analytical-First Principle. Full source lives outside the SDK, in an external research repository under `simulation_experiments/2026-04-13-perigen-efm-early-warning/`.

### The Decision Question

"Does HEALTH_SYSTEM_NAME have sufficient volume and evaluation design to determine whether PeriGen PeriWatch Vigilance reduces neonatal HIE (hypoxic-ischemic encephalopathy) at a pre-specified effect size, and can we do so without widening existing disparities?"
...
- **H2 (HIE as Scorecard metric):** CONFIRMED NO. Fraction of cells where HIE is detectable at E = 18 and A = 0.0017 (the lower-bound literature rate) = 0.0. Recommendation: HIE must be a sentinel safety outcome, not a primary scorecard metric, unless the post period or denominator grows substantially.
...
**Equity arm (Phase 0):** Group B (Medicaid + minority, 25% of volume) assigned baseline × 1.5, sensitivity − 0.10, action rate − 0.15. ΔD = RRR_A − RRR_B computed per cell. Verdict at H1-qualifying settings: meaningful disparity risk persists unless action rate parity is improved.
...
- **Time-to-action coupling.** The analytical chain treats action_rate and efficacy as independent constants. In reality, earlier alerts produce more lead time, which may produce higher efficacy at equal nominal action rate.
- **Alert fatigue half-life.** Action rate was treated as a single point estimate per cell. The Kumar half-life model in the `sepsis_early_alert` reference scenario shows that action_rate can decay over months when alert burden rises.
- **Score-distribution-dependent effects.** PeriGen's operating point is fixed by the vendor, so threshold selection is off the table for this study. But if a future question asks "should we move threshold to improve NNA?", this analytical pass is no longer sufficient.
- **Analytic method rehearsal.** The Phase-0 results project ITS power on a simplified two-proportion basis. A genuine ITS rehearsal — segmented regression on monthly event counts with autocorrelated error — requires synthetic monthly data generation and a simulation harness.
...
2. **The gap list drives Phase 1, not enthusiasm.** If a future question about PeriGen depends on time-to-action coupling or alert fatigue, escalate to SDK with the `sepsis_early_alert` reference scenario and monthly operator dynamics turned on.
3. **The equity arm was feasible analytically because the question was point-estimate deltas.** If the equity question becomes "how does subgroup calibration shift the operating point?", it would require score-distribution fidelity and therefore simulation.
